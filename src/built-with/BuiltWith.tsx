...
export default function BuiltWith() {
  const orderedTools = useMemo(() => {
    const orderMap = new Map(
      toolOrder.map((id, index) => [id, index]),
    );
   ...
   */
  const revealTimeoutRef = useRef<number | null>(
    null,
  );
  ...
  const updateNavigationState = useCallback(() => {
    ...
    const firstCardIsAtStart =
      firstItemRect.left >=
      carouselRect.left - tolerance;

    const lastCardIsAtEnd =
      lastItemRect.right <=
      carouselRect.right + tolerance;

    setCanScrollLeft(!firstCardIsAtStart);
    setCanScrollRight(!lastCardIsAtEnd);
  }, []);
   ...
   */
  const ensureToolIsFullyVisible = useCallback(
    (toolId: string) => {
      const carousel = carouselRef.current;
      const toolItem =
        toolItemRefs.current.get(toolId);

      if (!carousel || !toolItem) {
        return;
      }
      ...
      const visibleLeftBoundary =
        carouselRect.left + safePadding;

      const visibleRightBoundary =
        carouselRect.right - safePadding;

      let horizontalAdjustment = 0;

      if (toolRect.left < visibleLeftBoundary) {
        horizontalAdjustment =
          toolRect.left - visibleLeftBoundary;
      } else if (
        toolRect.right > visibleRightBoundary
      ) {
        horizontalAdjustment =
          toolRect.right - visibleRightBoundary;
      }

      if (Math.abs(horizontalAdjustment) < 1) {
  ...
  }, [activeToolId, updateNavigationState]);
  ...
  const activateTool = (tool: Tool) => {
     ...
     */
    window.requestAnimationFrame(() => {
      ensureToolIsFullyVisible(tool.id);
    });
  ...
  /*
   * Converts a vertical mouse-wheel movement into
   * horizontal carousel scrolling.
   *
   * Native horizontal trackpad and touch swiping remain
   * available through overflow-x-auto.
   */
  const handleWheel = (
